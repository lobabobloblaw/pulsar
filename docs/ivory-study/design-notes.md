...
| --- | --- | --- |
| Two pulse channels, triangle, noise, DPCM | Five fixed named tracker lanes in canonical order. No generic oscillator rack, filter knobs, ADSR, effects rack, extra synth engines, or per-channel gain faders are implied. | `src/tracker/model/types.ts`, `src/audio/core/` |
