 ...
 *
 *  LEVELS — open, deliberately not solved here. Eight voices are louder than four: at
 *  pulsar's render gain (DEFAULT_MASTER_GAIN 2.0, the app knob at maximum) the two-pass
 *  mix measures -16.62 dBFS RMS and its unclamped peak is 1.1623, so 38 samples out of
 *  13.2 million clamp, in 21 isolated spots. Gate C allows 8 and therefore FAILS. The
 *  piece is left at the composed levels anyway: the source project's own render of this
 *  same document peaks at 0.6063, almost exactly half, so this is the render gain and not
 *  the arrangement, and nothing clips at any normal listening level. Scaling the volume
 *  columns would quantise every voice onto the 0..15 grid and change their balance;
 *  lowering the core's gain would re-pin every golden checksum and quieten live play. The
 *  decision belongs to whoever owns the render gain, not to this converter.
 */
